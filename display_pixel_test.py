...
# --- WATCH STT ENDPOINT CONFIG (added) ---
# The SenseCAP Watcher uploads a WAV here and expects {"text": "..."} back.
STT_HTTP_PORT = 8000
...
# Serialize all access to the Whisper model so the watch's HTTP requests and the
# continuous wake-word loop never run inference at the same time.
...
def play_beep(frequency, duration_ms):
    try:
        ...
        fade_window = np.ones(len(wave_data))
        fade_window[:fade] = np.linspace(0, 1, fade)
        fade_window[-fade:] = np.linspace(1, 0, fade)
        wave_data = wave_data * fade_window
        
        sd.play(wave_data.astype(np.float32), sample_rate, device=OUTPUT_DEVICE_INDEX)
        sd.wait()
    except Exception:
        pass
...
# ─────────────────────────────────────────────
# WATCH STT SERVER (added)
# Reuses the already-loaded whisper_model. Runs in a background thread so it
# doesn't disturb the wake-word loop below.
# ─────────────────────────────────────────────
stt_app = Flask(__name__)
logging.getLogger("werkzeug").setLevel(logging.WARNING)  # quiet request logs
...
def _transcribe_upload():
    ...
    try:
        ...
        return jsonify({"error": f"bad wav: {e}"}), 400

    audio_fp32 = audio_i16.astype(np.float32) / 32768.0
    with whisper_lock:
        result = whisper_model.transcribe(audio_fp32, fp16=False, language="en")
...
def start_stt_server():
    def _run():
        stt_app.run(host="0.0.0.0", port=STT_HTTP_PORT, threaded=True)
    threading.Thread(target=_run, daemon=True).start()
    print(f"⌚ Watch STT endpoint up on 0.0.0.0:{STT_HTTP_PORT}")
...
if __name__ == "__main__":
    # Bring the watch STT endpoint up first so the watch can transcribe even
    # while the hardware peripherals are still being discovered.
    start_stt_server()
    try:
